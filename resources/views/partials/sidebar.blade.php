<div class="col-md-4 col-xl-3">
    <div class="sidebar px-4 py-md-0">
        ...
        <form class="input-group" action="" method="GET">
            <input type="text" class="form-control" name="s" placeholder="Search" value="{{ request('s') }}">
        ...
        <div class="row link-color-default fs-14 lh-24">
            @foreach($categories as $category)
                <div class="col-6"><a href="{{route('blog.category',$category->id)}}{{request('s') ? '?s=' . request('s') : ''}}">{{$category->name}}</a></div>
            @endforeach
        </div>
        ...
        <div class="gap-multiline-items-1">
            @foreach($tags as $tag)
                <a class="badge badge-secondary" href="{{route('blog.tag',$tag->id)}}{{request('s') ? '?s=' . request('s') : ''}}">{{$tag->name}}</a>
            @endforeach
        </div>
